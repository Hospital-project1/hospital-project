      ...
      <div className="bg-white p-6 rounded-lg shadow-lg">
              ...
              >
                ...
                {doctors.map((doc) => (
                  <option key={doc._id} value={doc._id}>
                    د. {doc.user.name} ({doc.specialty || "General"})
                  </option>
                ))}
              </select>
              {errors.doctor && (
        ...
        <div className="mt-8 p-4 bg-gray-100 rounded-lg">
          ...
          <div className="max-h-60 overflow-y-auto bg-white rounded p-2">
            {debugLog.length === 0 ? (
              <p className="text-gray-500 text-sm">No debug entries yet</p>
            ) : (
              debugLog.map((log, index) => (
                <div key={index} className="mb-2 last:mb-0 border-b pb-2">
                  <div className="flex justify-between">
                    <span className="text-sm font-medium">
                      [{new Date(log.timestamp).toLocaleTimeString()}]
                    </span>
                    <span className="text-xs text-gray-500">
                      {index + 1}/{debugLog.length}
                    </span>
                  </div>
                  <p className="text-sm">{log.message}</p>
                  {log.data && (
                    <pre className="text-xs mt-1 p-1 bg-gray-50 rounded overflow-x-auto">
