...
// Handler for GET request to get all doctors with pagination
export async function GET(request) {
  try {
    // Connect to database
    await connectDB();
    ...
    const viewMode = searchParams.get('viewMode') || 'slot';
    
    // Set the limit based on the view mode (3 for card, 5 for slot)
    const effectiveLimit = viewMode === 'card' ? 3 : 5;
    ...
    // Get paginated doctors with populated user fields
    const doctors = await Doctor.find()
      .populate('user', '-password') // Exclude password field
      .skip(skip)
